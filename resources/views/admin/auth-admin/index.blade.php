                          ...
                          <tbody>
                              ...
                              <tr>
                                  <td> <input type="checkbox" name="check[]"
                                    value="{{ $admin->id }}" id=""> {{ $loop->index + 1 }}</td>
                                  <td>{{ $admin->Name }}</td>
                                  <td>{{ $admin->Level_price	 }}</td>
        
                                  <td>
                                      <!-- Add button to trigger Edit popup modal -->
                                      <button type="button" class="bg-none" data-bs-toggle="modal" data-bs-target="#editModal{{ $admin->id }}">
                                        <i class="fa-sharp fa-solid fa-pen-to-square"></i>
                                      </button>

                                      <form method="POST" action=" {{ route('auth-admin.destroy', ['auth_admin' => $admin->id]) }} " onsubmit="return ConfirmDelete(this)" class="p-0" style="display: inline;">
                                        @method('DELETE')
                                        @csrf
                      ...
                      <div class="modal fade" id="editModal{{ $admin->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $admin->id }}" aria-hidden="true">
                          <div class="modal-dialog">
                              <div class="modal-content">
                                  ...
                                  <form method="POST" action="{{ route('auth-admin.update', ['auth_admin' => $admin->id]) }}">
                                      @csrf
                                      @method('PUT')
                                      <div class="modal-header">
                                          <h5 class="modal-title" id="editModalLabel{{ $admin->id }}">Edit Artist Level {{ $admin->id }}</h5>
                                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                      </div>
                                      <div class="modal-body">
                                          <!-- Add form fields with pre-filled data from $admin -->
                                          <div class="form-group">
                                              <label for="name">Name</label>
                                          ...
                                          <div class="form-group">
                                              ...
                                              <textarea class="form-control" id="level_price" name="level_price">{{ $admin->Level_price }}</textarea>
                                          </div>
                                          <!-- Add more fields as needed -->
                                      </div>
                                      <div class="modal-footer">
