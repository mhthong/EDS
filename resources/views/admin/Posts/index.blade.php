                        ...
                        <tbody>
                          ...
                          <tr>
                            <td class="p-1"><input type="checkbox" name="check[]" value="{{$row->id}}" id="">
                                {{ $stt++ }}</td>
                            <td>{{ $row->name }}</td>
                            <td> <img  src="{{ asset($row->image) }}" alt="" height="60px" width="60px"></td>
                            ...
                            <td style="text-align: center"> 
                                ...
                                @endif
                                @endisset</td>
                                <td>         @isset( $Menus_category[$row->id])
                            ...
                            <td style="text-align: center">
                              ...
                              <form method="POST" action="{{ route('posts.delete', ['id' => $row->id]) }}" onsubmit="return ConfirmDelete(this)" class="p-0" style="display: inline;">
                                @method('DELETE')
                                @csrf
