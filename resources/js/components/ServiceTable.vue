    ...
    <v-app>
      <v-card flat title="Nutrition">
        <div class="mb-3" style="display: flex; gap: 1rem; flex-wrap: wrap">
          ...
          <label style="padding-top: 12px; margin-top: 4px">
            <select
              class="form-control"
              id="showroomSelect"
              v-model="Status"
          ...
          <label style="padding-top: 12px; margin-top: 4px">
            <select
              ...
              style="padding: 5px; min-width: 170px; max-width: 170px"
            >
              <option :value="null" selected>Select Group Service</option>
              <option
                v-for="datagroupindex in datagroup"
                :key="datagroupindex.id"
                :value="datagroupindex.Name"
              >
                {{ datagroupindex.Name }}
              </option>
            </select>
        ...
        >
          <template v-slot:item="{ item }">
            <tr>
              ...
              <td @click="redirectToEditPage(item.id)">{{ item.group }}</td>
              <td
                @click="redirectToEditPage(item.id)">
                {{ item.name }}
              </td>

              <td @click="redirectToEditPage(item.id)">{{ item.time }} Hour</td>
              <td @click="redirectToEditPage(item.id)">{{ item.price }}</td>
              <td @click="redirectToEditPage(item.id)">{{ item.sale_price }}</td>
  ...
  methods: {
    redirectToEditPage(Id) {
      ...
      if (this.adminId != null) {
        const editUrl = `/admin/services/${Id}/edit`;
    ...
    AddNew() {
      if (this.adminId != null) {
        const editUrl = `/admin/services/create`;
        window.location.href = editUrl;
      }
    },
    generateTableData() {
    ...
    editItem(item) {
      ...
      this.editedItem = this.filteredData.find(
    ...
    deleteItemConfirm() {
      axios
        .delete(`/api/delete_service/${this.DeletedItem}`)
        .then((response) => {
  ...
  computed: {
    filteredData() {
      ...
      const filters = [
        ...
        {
          condition: (item) =>
            item.group_service.name
              .toLowerCase()
              .includes(this.selectedGroup?.toLowerCase()),
          key: "selectedGroup",
        },
