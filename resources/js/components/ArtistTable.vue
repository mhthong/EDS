        ...
        >
          <template v-slot:item="{ item }">
            <tr>
              ...
              <td  @click="redirectToEditPage(item.id)" :class="{'artist_pay_0': parseInt(item.artist_pay) === 0, 'artist_pay_1': parseInt(item.artist_pay) === 1}">{{ item.name }}</td>

              <td  @click="redirectToEditPage(item.id)">{{ item.fullname }}</td>
...
export default {
  ...
  methods: {
    ...
    AddNew() {
      if (this.adminId != null) {
        const editUrl = `/admin/artist/create`;
        window.location.href = editUrl;
      }
    },
    generateTableData() {
    ...
    deleteItemConfirm() {
      axios
        .delete(`/api/delete_artist/${this.DeletedItem}`)
        .then((response) => {
...
filteredData() {
      ...
      const conditions = [
        ...
        (item) => item.fullname.toLowerCase().includes(query),
