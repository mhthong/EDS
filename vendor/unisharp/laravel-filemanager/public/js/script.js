...
function createPagination(paginationSetting) {
  ...
  $('#search-button').click(function () {
    ...
    searchItemsAcrossPages(1, searchTerm);
  });

  // Other existing code...

  function searchItemsAcrossPages(page, searchTerm) {
    var totalPages = 1;
      ...
      if (page <= totalPages) {
        loadItems(page, searchTerm)
          .done(function (data) {
            ...
            totalPages = response.paginator ? response.paginator.total_pages : 1;
            page++;
            loadNextPage();
          });
      }
  ...
  function loadItems(page, searchTerm) {
    ...
    return performLfmRequest('jsonitems', { show_list: show_list, sort_type: sort_type, page: page || 1, search: searchTerm || '' }, 'html')
      .done(function (data) {
        selected = [];
        var response = JSON.parse(data);
        ...
        if (hasPaginator) {
          ...
          $('#pagination a').on('click', function (event) {
            ...
            loadItems($(this).closest('li')[0].getAttribute('data-num'), searchTerm);
            return false;
          });
        }
